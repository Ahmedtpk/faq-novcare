...
export default function Header() {
  ...
  return (
    <header className="bg-[#03043A] mb-8">
      <nav aria-label="Global" className="mx-auto flex max-w-7xl items-center justify-between p-6 lg:px-8">
        <div className="flex lg:flex-1">
          <Link href="/" className="-m-1.5 p-1.5">
            <span className="sr-only">Your Company</span>
            <Image alt="Novcare Logo" src="/logo.svg" width={128} height={32} className="h-8 w-auto" />
          </Link>
        </div>
        ...
        <PopoverGroup className="hidden lg:flex lg:gap-x-12">
          <Popover className="relative">
            ...
            >
              <div className="p-4 bg-[#1b4568]">
                {products.map((item) => (
                  <div
                    key={item.name}
                    className="group relative flex items-center gap-x-6 rounded-lg p-4 text-sm/6 hover:bg-[#03043A]"
                  >
                    <div className="flex size-11 flex-none items-center justify-center rounded-lg bg-gray-50 group-hover:bg-white">
                      <item.icon aria-hidden="true" className="size-6 text-black group-hover:text-indigo-600" />
                    </div>
            ...
            <div className="-my-6 divide-y divide-gray-500/10">
              <div className="space-y-2 py-6">
                <Disclosure as="div" className="-mx-3">
                  <DisclosureButton className="group flex w-full items-center justify-between rounded-lg py-2 pr-3.5 pl-3 text-base/7 font-semibold text-gray-900 hover:bg-gray-100">
                    Product
                    <ChevronDownIcon
                      aria-hidden="true"
                      className="size-5 flex-none text-gray-500 group-data-open:rotate-180"
                    />
                  </DisclosureButton>
